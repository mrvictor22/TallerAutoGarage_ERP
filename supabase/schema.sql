...
CREATE POLICY "Admins can update all profiles" ON profiles FOR UPDATE TO authenticated USING (auth.is_admin());
CREATE POLICY "Admins can insert profiles" ON profiles FOR INSERT TO authenticated WITH CHECK (auth.is_admin());
...
CREATE POLICY "Authorized roles can manage payments" ON payments FOR ALL TO authenticated USING (auth.user_role() IN ('admin', 'reception', 'mechanic_lead')) WITH CHECK (auth.user_role() IN ('admin', 'reception', 'mechanic_lead'));
...
CREATE POLICY "Admins can manage templates" ON whatsapp_templates FOR ALL TO authenticated USING (auth.is_admin());

-- WhatsApp messages policies
CREATE POLICY "Authenticated users can view messages" ON whatsapp_messages FOR SELECT TO authenticated USING (true);
CREATE POLICY "Admin and reception can send messages" ON whatsapp_messages FOR INSERT TO authenticated WITH CHECK (auth.user_role() IN ('admin', 'reception'));
...
CREATE POLICY "Admins can manage config" ON workshop_config FOR ALL TO authenticated USING (auth.is_admin());
...
CREATE POLICY "Admins can manage expense categories" ON expense_categories FOR ALL TO authenticated USING (auth.is_admin());

-- Expenses policies
CREATE POLICY "Authenticated users can view expenses" ON expenses FOR SELECT TO authenticated USING (true);
CREATE POLICY "Admin and reception can manage expenses" ON expenses FOR ALL TO authenticated USING (auth.user_role() IN ('admin', 'reception'));

-- Audit log policies
CREATE POLICY "Admins can view audit log" ON audit_log FOR SELECT TO authenticated USING (auth.is_admin());
